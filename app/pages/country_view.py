...
def show(df: pd.DataFrame):
    st.title("Country & City Explorer")
    ...
    col3.metric("Cheap Meal", f"${city_data['meal_cheap_restaurant']:,.1f}")
    ...
    fig_compare.add_trace(go.Bar(
        ...
        marker_color='#2563EB'  # Harmonisé avec le primaryColor du thème (Bleu pro)
    ))
    fig_compare.add_trace(go.Bar(
        name=f"{selected_country} avg", x=compare_cols, y=country_avg,
        marker_color='#94A3B8'  # Gris neutre pro
    ))
